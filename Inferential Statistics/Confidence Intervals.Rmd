...
```{r set-seed}
set.seed(02161993)                 # make sure to change the seed
```
...
samp %>% summarize(mean = mean(area),iqr = IQR(area),med = median(area),sd = sd(area))
...
# type your code for the Exercise here, and Knit
Yes the true average lies in the confidence interval.
```
...
ggplot(data = ci_99_data, aes(x = bound, y = replicate, 
                           group = replicate, color = capture_mu)) +
  geom_point(size = 2) +  # add points at the ends, size = 2
  geom_line() +           # connect with lines
  geom_vline(xintercept = params$mu, color = "darkgray") # draw vertical line
