...
ggplot(data=sfo_feb_flights,aes(x = arr_delay)) + geom_histogram(binwidth = 100)
```
...
sfo_feb_flights %>% group_by(carrier) %>% summarize(median_ad = median(arr_delay), IQR_ad = IQR(arr_delay))
```
...
nycflights <- nycflights %>% mutate(air_time_hr = air_time/60)
nycflights <- nycflights %>% mutate(avg_speed = distance/air_time_hr) 
...
flights <- nycflights  %>% group_by(dep_type,arr_type) %>% summarise(count = n()) %>%  mutate(perc = count/sum(count))
ggplot(data=flights,aes(x=dep_type,y=perc*100,fill=arr_type)) +geom_bar(stat = "identity")
```
